  ...
  const fetchNewArrivals = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const sorted = response.data.jewellery
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <Link to="/jewellery?sort=new" style={styles.viewAll}>
      ...
      <div className="scroll-container" style={styles.scrollContainer}>
        {jewellery.map((item) => {
          ...
          const isNew = isNewArrival(item.createdAt);
          ...
          return (
            ...
            >
                <Link 
                    to={`/jewellery/${item._id}`} 
                    className="text-decoration-none"
                  
                  >
              <div style={styles.imageContainer}>
                <img 
                  src={item.image || 'https://via.placeholder.com/280x280'} 
                  alt={item.name}
                  style={styles.image}
                  onError={(e) => e.target.src = 'https://via.placeholder.com/280x280'}
                />
                
                {isNew && <span style={styles.JnewBadge}>✨ NEW</span>}
                {discountPercent > 0 && (
...
export default NewArrivalsJewellery;